...
createExcel1 <- function(data) {
  ...
  for (i in sort(unique(Hospital$Year_Month))){
    ...
    saveWorkbook(wb, paste0(outdir, "/output/Investigations per hospital per month.xlsx"),
               overwrite = TRUE)}
}


createExcel2 <- function(data) {
  wb = createWorkbook(paste0(outdir, "/output/Investigations Each Month By Hospital"))
  Hospital <- data[c("Hospital", "Investigation", "Year_Month")]
  for (i in sort(unique(Hospital$Investigation))){
    ...
    saveWorkbook(wb, paste0(outdir, "/output/Investigations per month per hospital.xlsx"),
                 overwrite = TRUE)}
}
