...
lapply(sort(unique(turnoverTime$Investigation)), 
  ...
  labs(title = i,
    subtitle = "Outliers, defined as ±1.5*IQR, have not been plotted. Boxplots \nrepresent Median, Q1-Q3 and 1.5*Q1Q3. Red line is the mean.",
  ...
  scale_x_discrete(limits = sort(unique(auditNoNAs$Year_Month)), breaks = sort(unique(auditNoNAs$Year_Month))) +    
  ylim(-5, quantile(turnoverTime[turnoverTime$Investigation == i,][[3]])[[4]]+10) +
  geom_hline(yintercept = auditNoNAs$Target_Turnaround, linetype = "dashed", 
    color = "red", size = .5)
})
...
Hospital <- Audit[c("Hospital", "Investigation", "Year_Month")]
...
workFreq <- auditNoNAs[c("Billing_Category", "Year_Reported", "Month_Reported")]
